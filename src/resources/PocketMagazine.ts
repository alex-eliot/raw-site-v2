...
export default class PocketMagazineHandler implements ResourceHandler {
  url: URL;
  id: string
  ...
  private calculateHash(params: Record<string,string>, seed: string): string {
  ...
  const mangahash = crypto.SHA256(hashtable.join(',')).toString(crypto.enc.Hex);
  return crypto.SHA512(mangahash + seed).toString(crypto.enc.Hex);
  }

  //Main execution function
  async execute(): Promise<JSZip> {
    const mangahash = this.calculateHash({
      "platform": "3",
      "episode_id": this.id.toString()
    },
    [crypto.SHA256(''), crypto.SHA512('')].join('_')
  )

    const resp = await getFromProxy(
      `https://api.pocket.shonenmagazine.com/web/episode/viewer?platform=3&episode_id=${this.id}`,
      {
        'x-manga-hash': mangahash,
      }
    )
